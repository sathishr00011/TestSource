...
CALL "C:\Program Files (x86)\7-Zip\7z.exe" x -y %TOFOLDER%\%NEWESTFILE% -o%LOCALLANGUAGEDATAFOLDER% *.xml -r > NUL
...
IF %FORGEMODE%=="FULL" (
	ECHO [%date% %time%] Call Scripts for Local language for cluster 1 - Baseline Update  >> CON
	ECHO [%date% %time%] Call Scripts for Local language for cluster 1 - Baseline Update  2>&1
	CD %LOCALLANGUAGEINDEXFOLDER%\CONTROL\
	...
	IF %ERRORLEVEL% GEQ 1 GOTO LOCALLANGCLUSTER1ERROR
	
	ECHO [%date% %time%] Extracting files for forge for Cluster 2 >> CON
	ECHO [%date% %time%] Extracting files for forge for Cluster 2 2>&1
	"C:\Program Files (x86)\7-Zip\7z.exe" x -y %TOFOLDER%\%NEWESTFILE% -o%LOCALLANGUAGEDATAFOLDER% *.xml -r >ForgeFileList.txt
        
        
	ECHO [%date% %time%] Call Scripts for Local language for cluster 2 - Baseline Update >> CON
	ECHO [%date% %time%] Call Scripts for Local language for cluster 2 - Baseline Update 2>&1
	CD %LOCALLANGUAGEINDEXFOLDER%\CONTROL\
	set_baseline_data_ready_flag.bat
	IF %ERRORLEVEL% GEQ 1 GOTO FORGEFAIL
	...
	baseline_apply_index_cluster2.bat
	IF %ERRORLEVEL% GEQ 1 GOTO LOCALLANGCLUSTER2ERROR
	...
	REM Delete any existing files in the forge folders
	DEL %LOCALLANGUAGEDATAFOLDER%\*.xml 2>NUL
    DEL %ENGLISHDATAFOLDER%\*.xml 2>NUL
	
	REM Delete dummy file 
	DEL ForgeFileList.txt
	
    ECHO [%date% %time%] Extracting files for forge for Cluster 2 >> CON
	ECHO [%date% %time%] Extracting files for forge for Cluster 2 2>&1	
    "C:\Program Files (x86)\7-Zip\7z.exe" x -y %TOFOLDER%\%NEWESTFILE% -o%ENGLISHDATAFOLDER% *.xml -r >ForgeFileList.txt
    ECHO [%date% %time%] Call Scripts for English language for cluster 2 - Baseline Update >> CON
	ECHO [%date% %time%] Call Scripts for English language for cluster 2 - Baseline Update 2>&1
	CD %ENGLISHINDEXFOLDER%\CONTROL\
	set_baseline_data_ready_flag.bat
	IF %ERRORLEVEL% GEQ 1 GOTO FORGEFAIL
	...
	baseline_apply_index_cluster2.bat
	IF %ERRORLEVEL% GEQ 1 GOTO ENGCLUSTER2ERROR
...
IF %FORGEMODE%=="PARTIAL" (
        ECHO [%date% %time%] Call Scripts for Local language - Partial Update 2>&1
	CD %LOCALLANGUAGEINDEXFOLDER%\CONTROL\
