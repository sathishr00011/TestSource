...
IF %FORGEMODE%=="FULL" (
	...
	DEL %LOCALLANGUAGEDATAFOLDER%\*.xml 2>NUL
	REM DEL %ENGLISHDATAFOLDER%\*.xml 2>NUL	
	...
	REM "C:\Program Files (x86)\7-Zip\7z.exe" x -y %TOFOLDER%\%NEWESTFILE% -o%LOCALLANGUAGEDATAFOLDER% *.xml -r >ForgeFileList.txt
	REM "C:\Program Files (x86)\7-Zip\7z.exe" x -y %TOFOLDER%\%NEWESTFILE% -o%ENGLISHDATAFOLDER% *.xml -r >ForgeFileList.txt
	...
	REM ECHO [%date% %time%] Call Scripts for Local language for cluster 2 - Baseline Update >> CON
	REM ECHO [%date% %time%] Call Scripts for Local language for cluster 2 - Baseline Update 2>&1
	REM CD %LOCALLANGUAGEINDEXFOLDER%\CONTROL\
	REM set_baseline_data_ready_flag.bat
	REM IF %ERRORLEVEL% GEQ 1 GOTO FORGEFAIL
	...
	REM baseline_apply_index_cluster2.bat
	REM IF %ERRORLEVEL% GEQ 1 GOTO LOCALLANGCLUSTER2ERROR
...
IF %FORGEMODE%=="PARTIAL" (
	ECHO [%date% %time%] Call Scripts for Local language - Partial Update > CON
