...
#$WebProjectpath="D:\IGHS Manifest Samples\ChefManifest\Projectweb.config\Web.config"
...
#$Applicationparameterfile="D:\IGHS Manifest Samples\Applicationparameterfile\DEV"
#net use "\\inf01nas01.dotcom.tesco.org" XXXXXXXX /user:dotcom\jv37

$Applicationparameterfile=$env:Appfile

#$cookbooklocation="G:\Chef\IGHS\chef-repo\cookbooks\authentication_service"


$cookbooklocation=$env:cookbook

$webconfigfile_ce=$cookbooklocation+"\templates\dbt\ce\Web.config.erb"
$webconfigfile_ap=$cookbooklocation+"\templates\dbt\ap\Web.config.erb"



$Applicationparameterfiles= ($Applicationparameterfile+"\CE\23.0\Manifest_IGHS_UK_CE_DEV_Application_Parameters.xml"),
           ($Applicationparameterfile+"\AP\23.0\Manifest_IGHS_UK_AP_DEV_Application_Parameters.xml")

#$attributefile = $cookbooklocation+"\attributes\dbt_configuration.rb"

#Set-Content $attributefile "if node.chef_environment == 'dbt"

#$cookbookName = "authentication_service"
...
$nodelist=$doc.SelectNodes('/Application/Environment/Services/Service[@name="AppStore.Authentication.Service"]/Web.Config/LoggingConfiguration/Listeners')
##
...
## Application prameter file ##
$nodelist1=$doc.SelectNodes('/Application/Environment/Services/Service[@name="AppStore.Authentication.Service"]/Web.Config/LoggingConfiguration/CategorySources')
##
...
$nodename1=$_.Name
    ...
    if ($nodename1 -eq "TescoAuthenicationApp")
    {
    $value=$_.value
...
## Application prameter file ##
$nodelist2=$doc.SelectNodes('/Application/Environment/Services/Service[@name="AppStore.Authentication.Service"]/Web.Config/appSettings')
##
...
$doc.Application.Environment.Services.Service | % { 
        if ($_.name -eq "AppStore.Authentication.Service")
        { 
         $_.DBConnection | % {
         $namedb=$_.name 
