    ...
    public partial class TFSBuildDefinitionClone : Form
    {
            ...
            try
            {
                ...
                foreach (var build in buildDetails)
                {
                    ...
                    if (appSettings.ContainsValue(build.Name))
                    {
                        ...
                        buildDefinitionClone.Name = buildDefinitionClone.Name.Replace("Copy ofRTC4.0_", BuildDefName);
                        ...
                        buildDefinitionClone.ProcessParameters = buildDefinitionClone.ProcessParameters.Replace("4", "5");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //new
                Old_Branch = comboBox18.SelectedItem.ToString();
                New_Branch = comboBox17.SelectedItem.ToString();
                //new
                ...
                foreach (var build in buildDetails)
                {
                    ...
                    if (appSettings.ContainsValue(build.Name))
                    {
                        ...
                        //new
                        IProcessTemplate template = buildServer.QueryProcessTemplates(buildDefinition.TeamProject).FirstOrDefault(i => i.ServerPath == buildDefinition.Process.ServerPath.Replace(Old_Branch, New_Branch));

                        if (template == null)
                        {
                            template = buildServer.CreateProcessTemplate(buildDefinition.Process.TeamProject, buildDefinition.Process.ServerPath.Replace(Old_Branch, New_Branch));
                            template.SupportedReasons = buildDefinition.Process.SupportedReasons;
                            template.Description = buildDefinition.Process.Description;
                            template.TemplateType = buildDefinition.Process.TemplateType;
                            template.Save();
                            ...
                            FieldInfo fieldInfo = object1.GetType().GetField("m_serverPath", BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.SetField | BindingFlags.DeclaredOnly | BindingFlags.Default | BindingFlags.ExactBinding | BindingFlags.FlattenHierarchy | BindingFlags.GetField | BindingFlags.GetProperty | BindingFlags.IgnoreCase |
                                  BindingFlags.IgnoreReturn | BindingFlags.Instance | BindingFlags.InvokeMethod | BindingFlags.NonPublic | BindingFlags.OptionalParamBinding |
                                    BindingFlags.Public | BindingFlags.PutDispProperty | BindingFlags.PutRefDispProperty | BindingFlags.SetField | BindingFlags.SetProperty | BindingFlags.Static | BindingFlags.SuppressChangeType);
                            fieldInfo.SetValue(buildDefinition.Process, buildDefinition.Process.ServerPath.Replace(Old_Branch, New_Branch));
                        }
                        template.Save();
                        //new
                        buildDefinition.Process = template;
                        buildDefinition.ProcessParameters = buildDefinition.ProcessParameters;
                        //new
                        ...
                        //    MappingType = MappingType.Replace(Old_Branch, New_Branch);




                        //    //buildDefinitionClone.Workspace.AddMapping(mapping.ServerItem, mapping.LocalItem, mapping.MappingType, mapping.Depth);
                        //    buildDefinition.Workspace.AddMapping(sourcecontrolpath, agentpath, mapping.MappingType, mapping.Depth);
                        //}

                        for (int i = 0; i < buildDefinition.Workspace.Mappings.Count(); i++)
                        {
                            IWorkspaceMapping mapping = buildDefinition.Workspace.Mappings[0];
                            if (mapping.LocalItem != null)
                            {
                                agentpath = mapping.LocalItem.ToString();
                                //agentpath = agentpath.Replace("Release8.0", "Release9.0");
                        ...
                        buildDefinition.ProcessParameters = buildDefinition.ProcessParameters.Replace(Old_Branch, New_Branch);
