...
namespace UpdateRootManifest
{
    public class ScheduleDeploy
    {
        ...
        public  void UpdateRootManifestSchedule(string rootRMFile, string componentRMFile, string altertedFile)
        {
           ...
           // string releaseManifestChildNode = "Applications/Product/Services/Service/Component";
            string releaseManifestChildNode = "Product/Package";
            string fileName = string.Empty; ;

            string[] strArray = altertedFile.Split('_');

            foreach (string obj in strArray)
            {
                if (obj=="AppStore")
                {
                    releaseManifestChildNode = "Product/Service/Package";
                }
                ...
                foreach (XmlNode item in nodes)
                {
                    ...
                    string msiVersion = item.Attributes["Path"].Value;
                        ...
                        if (node != null)
                        {
                            ...
                            XmlNodeList matchingBuildOutputPaths = node.SelectNodes(releaseManifestChildNodeX + "[@Name ='" + trackName + "']/@Path");
        ...
        public void UpdateRootManifestSchedule(string rootRMFile, string componentRMFile, string altertedFile, string componentname)
        {
            ...
            if (nodes != null)
            {
                foreach (XmlNode item in nodes)
                {
                    ...
                    if (trackName.Contains(componentname))
                    {
